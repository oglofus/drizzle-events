...
test("insert: pre and post events run, data can be modified in pre-insert", async () => {
  ...
  const res = await manager.insert(users, "id", {
    id: 1,
    name: "Alice",
    meta: { a: 1 },
  });
...
test("insert: destructured cancel and issue work together", async () => {
  ...
  assert.deepEqual(res.issues, [
    { message: "Cannot insert this row.", path: ["name"] },
  ]);
});
...
test("update: merges objects when enabled and respects post-update rollback", async () => {
  ...
  const res = await manager.update(users, "id", 1, {
    /* will be overwritten by pre handler */
  });
  assert.equal(res.type, "success");

  // pre handler saw the original row
  assert.deepEqual(preSeenOld, {
    id: 1,
    name: "Alice",
    meta: { a: 1, arr: [1] },
  });
  ...
  const res2 = await manager.update(users, "id", 1, {
    name: "Changed",
    meta: { a: 999 },
  });
