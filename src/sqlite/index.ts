...
export class SQLitePreInsertEvent<T extends SQLiteTable> extends IssueEvent<
  InferSelectModel<T>
> {
...
export class SQLitePostInsertEvent<T extends SQLiteTable> extends IssueEvent<
  InferSelectModel<T>
> {
...
export class SQLitePreUpdateEvent<T extends SQLiteTable> extends IssueEvent<
  InferSelectModel<T>
> {
...
export class SQLitePostUpdateEvent<T extends SQLiteTable> extends IssueEvent<
  InferSelectModel<T>
> {
  private readonly _row: InferSelectModel<T>;
...
export class SQLitePreDeleteEvent<T extends SQLiteTable> extends IssueEvent<
  InferSelectModel<T>
> {
  private readonly _row: InferSelectModel<T>;
...
export class SQLitePostDeleteEvent<T extends SQLiteTable> extends IssueEvent<
  InferSelectModel<T>
> {
  private readonly _row: InferSelectModel<T>;
...
export class SQLiteEventManager<
  D extends SQLiteAsyncDatabase<"async", unknown, AnyRelations>,
> extends RawEventManager {
  ...
  public async update<T extends SQLiteTable>(
    ...
    primary_value_or_data:
      InferSelectModel<T>[keyof InferSelectModel<T>] | SQLiteUpdateSetSource<T>,
    ...
    let data =
      maybe_data === undefined
        ? (primary_value_or_data as SQLiteUpdateSetSource<T>)
        : maybe_data;
    ...
    const old_row = await (
      this._database
        ...
        .from(table)
        .where(where_result.where) as unknown as {
        get(): InferSelectModel<T> | undefined;
      }
    ).get();
  ...
  public async delete<T extends SQLiteTable>(
    ...
    if (post_response.event.isCancelled()) {
      if (this._config.rollback_on_cancel) {
        await this._database
          .insert(table)
          .values(row as unknown as InferInsertModel<T>)
          .execute();
      }
