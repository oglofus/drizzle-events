...
export class PgPreInsertEvent<T extends Table> extends IssueEvent<
  InferSelectModel<T>
> {
...
export class PgPostInsertEvent<T extends PgTable> extends IssueEvent<
  InferSelectModel<T>
> {
...
export class PgPreUpdateEvent<T extends PgTable> extends IssueEvent<
  InferSelectModel<T>
> {
...
export class PgPostUpdateEvent<T extends PgTable> extends IssueEvent<
  InferSelectModel<T>
> {
  private readonly _row: InferSelectModel<T>;
...
export class PgPreDeleteEvent<T extends PgTable> extends IssueEvent<
  InferSelectModel<T>
> {
  private readonly _row: InferSelectModel<T>;
...
export class PgPostDeleteEvent<T extends PgTable> extends IssueEvent<
  InferSelectModel<T>
> {
  private readonly _row: InferSelectModel<T>;
...
export class PgEventManager<
  D extends PgAsyncDatabase<PgQueryResultHKT, AnyRelations>,
> extends RawEventManager {
  ...
  public async insert<T extends PgTable>(
    ...
    const runInsert = async (
      database: PgAsyncDatabase<PgQueryResultHKT, AnyRelations>,
    ): Promise<Response<InferSelectModel<T>>> => {
      const issues: Issue[] = [];
      const pre_response = await this.run(
        table,
        "pre-insert",
        new PgPreInsertEvent<T>(issue_fields, data),
      );
    ...
    if (this._config.rollback_on_cancel) {
      try {
        ...
        if (error instanceof PgEventRollbackError) {
          return errorResponse(error.message, [...error.issues]);
        }
  ...
  public async insert_batch<T extends PgTable>(
    ...
    const runInsertBatch = async (
      ...
      for (let i = 0; i < data.length; i++) {
        const pre_response = await this.run(
          table,
          "pre-insert",
          new PgPreInsertEvent<T>(issue_fields, data[i]),
        );
        issues.push(...pre_response.event.issues);
        ...
        if (pre_response.event.isCancelled()) {
          ...
          return errorResponse(message, [...issues]);
        }
      ...
      for (let i = 0; i < results.length; i++) {
        const post_response = await this.run(
          table,
          "post-insert",
          new PgPostInsertEvent<T>(issue_fields, results[i]),
        );
        issues.push(...post_response.event.issues);
    ...
    if (this._config.rollback_on_cancel) {
      try {
        return await this._database.transaction(async (tx) =>
          runInsertBatch(tx, true),
        );
      } catch (error) {
        if (error instanceof PgEventRollbackError) {
          return errorResponse(error.message, [...error.issues]);
  ...
  public async update<T extends PgTable>(
    ...
    primary_value_or_data:
      InferSelectModel<T>[keyof InferSelectModel<T>] | PgUpdateSetSource<T>,
    ...
    const primary_value =
      maybe_data === undefined ? primary_field_or_value : primary_value_or_data;

    let data =
      maybe_data === undefined
        ? (primary_value_or_data as PgUpdateSetSource<T>)
        : maybe_data;
    ...
    if ("error" in primary_info) {
      return errorResponse(
        `${primary_info.error} Pass a primary_field explicitly.`,
      );
    }
    ...
    const runUpdate = async (
      ...
      if (!old_row) {
        return errorResponse("The row does not exist.", [...issues]);
      }

      const pre_response = await this.run(
        table,
        "pre-update",
        new PgPreUpdateEvent<T>(issue_fields, data, old_row),
      );
      issues.push(...pre_response.event.issues);
      ...
      if (this._config.merge_objects) {
        for (const key in old_row) {
          ...
          // @ts-ignore
          (data as Record<string, unknown>)[key] = deepMerge(
            old_row[key],
            (data as Record<string, unknown>)[key],
            this._config.array_strategy,
          );
        }
      }
      ...
      const post_response = await this.run(
        table,
        "post-update",
        new PgPostUpdateEvent<T>(issue_fields, row, old_row),
      );
      issues.push(...post_response.event.issues);
  ...
  public async update_batch<T extends PgTable>(
    ...
    const runUpdateBatch = async (
      ...
      for (const update of updates) {
        ...
        if ("error" in where_result) {
          if (rollbackOnError) {
            throw new PgEventRollbackError(where_result.error, [...issues]);
          }

          return errorResponse(where_result.error, [...issues]);
        }
      ...
      return successResponse(rows, [...issues]);
    };

    if (this._config.rollback_on_cancel) {
      try {
        return await this._database.transaction(async (tx) =>
          runUpdateBatch(tx, true),
        );
      } catch (error) {
        if (error instanceof PgEventRollbackError) {
          return errorResponse(error.message, [...error.issues]);
  ...
  public async delete<T extends PgTable>(
    ...
    const runDelete = async (
      ...
      const pre_response = await this.run(
        table,
        "pre-delete",
        new PgPreDeleteEvent<T>(issue_fields, row),
      );
      issues.push(...pre_response.event.issues);
      ...
      const post_response = await this.run(
        table,
        "post-delete",
        new PgPostDeleteEvent<T>(issue_fields, row),
      );
      issues.push(...post_response.event.issues);
  ...
  protected _resolvePrimaryKeys<T extends PgTable>(
    ...
    for (const primary_column of primary_columns) {
      const entry = Object.entries(table_columns).find(
        ([, column]) => column === primary_column,
      );
  ...
  protected _buildWhereFromPrimaryValue<T extends PgTable>(
    ...
    for (const key of keys) {
      if (!(key in (primary_value as Record<string, unknown>))) {
        return {
          error: `Composite primary key requires a value for "${String(key)}".`,
        };
      }
    }

    return {
      where: this._buildWhereFromKeys(
        table,
        keys,
        primary_value as Partial<InferSelectModel<T>>,
      ),
    };
  }
  ...
  protected _buildWhereFromKeys<T extends PgTable>(
    ...
    const clauses = keys.map((key) =>
      eq(table_columns[key], values[key] as any),
    );
  ...
  protected _getIssueFields<T extends PgTable>(table: T) {
    return Object.keys(getColumns(table)) as Extract<
      keyof InferSelectModel<T>,
      string
    >[];
  }
